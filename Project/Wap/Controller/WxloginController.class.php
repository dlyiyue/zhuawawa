...
class WxloginController extends \Think\Controller
{
	public function index()
	{
		...
		if ($ubeiopenid == '' && $sysconfig['cbeicode'] == 1) {
			$sysconfig['cbeicode'] = 1;
	...
	public function checkuser($uopenid = '', $ubeiopenid = '', $utid = 0, $uickname = '', $usex = 0, $uheadimgurl = '', $udizhi = '')
	{
		...
		$url = $config['cbeiurl'].'/wawa/jiawawa/index.html';
		header('Location:' . $url);
	}
	public function getbeiuopenid()
	{
		...
		$headurl = $sysconfig['cfaurl'] . U('Wap/Jiawawa/action?utid=' . $this->geturl($urls, 2) . '&ubeiopenid=' . $ubeiopenid);
	...
	public function mainfo()
	{
		...
		if ($config['cbeicode'] == 1) {
			$url = $config['cbeiurl'] . U('Wap/Jiawawa/action?utid=' . $utid);
		} else {
			$url = $config['cfaurl'] . U('Wap/Jiawawa/action?utid=' . $utid);
		}
